...
import java.util.concurrent.atomic.AtomicReference;
    ...
    public class LogTests extends TestCase {
        ...
        public void testMultiThreadingLog() throws InterruptedException {
            ...
            for (int i = 0; i < 30; i++) {
                threads.add(new Thread(()->Log.build().setSeverity(Log.Severity.Error).setInspectNeeded(true).event("TT")));
                threads.add(new Thread(()->Log.build().setSeverity(Log.Severity.Info).setInspectNeeded(true).event("DD")));
                threads.add(new Thread(()->Log.build().setSeverity(Log.Severity.Warning).setInspectNeeded(false).event("EE")));
            }
